...
<template>
  <chat-container>
    <div class="bg-white mb-8 mt-4 mb-20">
      <div class="grid grid-cols-1 pb-20">
        <div
          :style="{backgroundImage: `url(${background})`, zIndex:0}"
          class="relative z-0 px-4 pt-80 pb-10 pb-3 bg-fixed from-black bg-cover"
        >
          <h2
        ...
        <div class="col-start-1 row-start-2 px-4">
          <div class="flex items-center  font-medium my-5 ">
            ...
            <div class="ml-1">
              <span class="text-black">{{!isGetDeatilLoading ? detail.page_views : '数据加载中...'}}</span>
            </div>
          </div>
          <div class="flex items-center  font-medium my-5 ">
            ...
            <div class="ml-1">
              <span>{{!isGetDeatilLoading ? lib.FormatTimerSecond(detail.create_time) : '数据加载中...'}}</span>
            </div>
          </div>
          <hr class="w-16 border-gray-300 hidden" />
        </div>
        <div class="col-start-1 row-start-3 space-y-3 px-4">
          <template v-if="!isGetDeatilLoading">
