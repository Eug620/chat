      ...
      <div class="flex items-center justify-between flex-wrap">
        <div class="w-0 flex-1 flex items-center">
          ...
          <p class="ml-3 font-medium text-white truncate">
            <span class="md:hidden">{{ props.msgText }}</span>
            <span class="hidden md:inline">{{ props.msgText }}</span>
          </p>
        </div>
        <div class="order-2 flex-shrink-0 sm:order-3 sm:ml-3">
          <button
            @click="useCloseMessage"
            type="button"
...
export default {
  ...
  setup (props, ctx) {
    ...
    state.curremtVm = getCurrentInstance().proxy
    const {
      useCloseMessage
    } = useMessage(props, state, ctx)
